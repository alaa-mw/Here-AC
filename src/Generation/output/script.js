...
function renderProductListComponent() {
  const template = productState.products
    .map(
      (product) => `
...
function ngOnInitProductDetailsComponent(id) {
  if (id) {
    selectedProduct = productState.products.find((p) => p.id === id);
  }
}
...
function onSubmitAddProductComponent() {
  if (editingId) {
    productState.updateProduct({ id: editingId, ...formProduct });
  } else {
...
function showSection(section) {
  [addEditProductSection, productDetailsSection, productListSection].forEach(
...
const addEditProductSection = document.getElementById(
